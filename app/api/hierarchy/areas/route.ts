...
export async function GET(req: NextRequest) {
  const { error, orgId } = await withHierarchyGuard('area.view');
  if (error) return error;
  try {
    const result = await HierarchyService.listAreas(parseListParams(req, orgId));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const area = await HierarchyService.createArea(orgId, userId, body);
