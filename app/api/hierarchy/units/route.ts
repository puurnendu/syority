...
export async function GET(req: NextRequest) {
  const { error, orgId } = await withHierarchyGuard('unit.view');
  if (error) return error;
  try {
    const result = await HierarchyService.listUnits(parseListParams(req, orgId));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const unit = await HierarchyService.createUnit(orgId, userId, body);
