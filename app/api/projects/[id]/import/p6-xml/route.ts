...
import { parseP6Xml } from '@/modules/Scheduling/parsers/P6XmlParser';
...
export const POST = withTenantGuard(async (req: NextRequest, { params }, session) => {
  ...
  if (error) {
    console.error(`[P6-XML Import] Forbidden for user ${session?.user?.id}, role: ${(session?.user as any)?.role}, org: ${session?.user?.organization_id}`);
    return error;
  }
  ...
  try {
    ...
    console.time('[P6-XML] File Read');
    const xmlString = await file.text();
    console.timeEnd('[P6-XML] File Read');

    // ── 3. Parse ─────────────────────────────────────────────────────────────
    await setImportStatus(projectId, userId, 'Step 2/5: Parsing XML project structure...');
    console.time('[P6-XML] Parsing');
    let parsed: { 
      activities: Awaited<ReturnType<typeof parseP6Xml>>['activities']; 
      relationships: Awaited<ReturnType<typeof parseP6Xml>>['relationships'];
      wbsNodes: Awaited<ReturnType<typeof parseP6Xml>>['wbsNodes'];
    };
    try {
      parsed = await parseP6Xml(xmlString, projectId, orgId, workpackId);
    } catch (parseErr: any) {
      console.timeEnd('[P6-XML] Parsing');
    ...
    if (parsed.activities.length === 0) {
      await clearImportStatus(projectId, userId);
      return NextResponse.json({ error: 'No tasks found in XML' }, { status: 422 });
    }
    ...
    if (!resolvedSiteId) {
        await clearImportStatus(projectId, userId);
        console.error(`[P6-XML-Import] Failed to resolve site_id for project ${projectId}`);
    ...
    try {
      console.time('[P6-XML] DB Transaction');
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (!event) {
            event = await tx.event.create({
                data: {
                    ...
                    code:             `IMP-P6-${Date.now().toString().slice(-4)}`,
                    status:           'planning',
                }
            });
        }
        ...
        if (parsed.wbsNodes.length > 0) {
            ...
            // For P6, they might not be in order, so we might need multiple passes or bread-first
            // But usually the parser can handle it if we sort by some logic or just retry.
            // A safer way is to create all first then update parents, but we'll try outline order first.
            for (const node of parsed.wbsNodes) {
                const parentId = node.parent_external_id ? wbsExternalToDbId.get(node.parent_external_id) : null;
                const dbNode = await tx.wbsNode.create({
                    data: {
                        organization_id: orgId,
                        event_id:        event.id,
                ...
                wbsExternalToDbId.set(node.external_id, dbNode.id);
            }
        }

        // 2. Bulk Create NEW activities in chunks
        console.time(`[P6-XML] Bulk Create ${parsed.activities.length}`);
        ...
        console.timeEnd(`[P6-XML] Bulk Create ${parsed.activities.length}`);

        // Fetch back IDs for relationships in chunks to avoid PG parameter limits
        console.time('[P6-XML] Fetch IDs');
        ...
        return { activityCount: newlyCreated.length, relCount: relRows.length };
      }, {
      ...
      console.error('[P6-XML-Import] Transaction failed:', txErr);
      await clearImportStatus(projectId, userId);
      return NextResponse.json(
