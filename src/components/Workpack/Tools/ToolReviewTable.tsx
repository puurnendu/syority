...
import type { GeneratedTool } from './GenerateToolsModal';
...
type EditableTool = GeneratedTool & { _selected?: boolean };

interface ToolReviewTableProps {
  open: boolean;
  onClose: () => void;
  tools: GeneratedTool[];
  workpackId: string;
...
export function ToolReviewTable({
  open,
  onClose,
  tools: initialTools,
  workpackId,
  sourceLabel = 'AI — Rigging plan + Activities + Equipment',
  onSaved,
  onRegenerate,
}: ToolReviewTableProps) {
  const [rows, setRows] = useState<EditableTool[]>(() =>
    initialTools.map((t) => ({ ...t, _selected: true }))
  );
  const [categoryFilter, setCategoryFilter] = useState('All');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const updateRow = useCallback((index: number, field: keyof GeneratedTool, value: string | number | boolean | null) => {
    setRows((prev) => {
      const next = [...prev];
      const r = { ...next[index]! };
  ...
  const handleSave = async () => {
    const toSave = rows.filter((r) => r._selected !== false).map(({ _selected, ...t }) => ({
      ...t,
      toolType: t.toolType,
    }));
    if (toSave.length === 0) {
      setError('Select at least one tool to add.');
      return;
    }
    setError('');
    setSaving(true);
    try {
      const res = await fetch(`/api/workpacks/${workpackId}/tools/bulk-create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tools: toSave }),
      });
      if (!res.ok) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">
              Review Generated Tools ({rows.length} items)
            </h3>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-gray-200 text-xs text-gray-500 uppercase tracking-wider">
                ...
                <th className="py-2 pr-2 text-left font-medium" style={{ minWidth: 180 }}>Tool name</th>
                <th className="py-2 pr-2 text-left font-medium" style={{ minWidth: 200 }}>Description</th>
                <th className="py-2 pr-2 text-right font-medium w-16">Qty</th>
                <th className="py-2 pr-2 text-left font-medium w-24">Type</th>
                <th className="py-2 pr-2 text-center font-medium w-16">Cert</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((row, i) => {
                  ...
                  >
                    ...
                    <td className="py-1.5 pr-2">
                      <input
                        ...
                        onChange={(e) => updateRow(globalIndex, 'description', e.target.value || null)}
                        className="w-full px-2 py-1 border border-gray-200 rounded text-xs"
                    ...
                    <td className="py-1.5 pr-2 text-right">
                      <input
                        ...
                        min={1}
                        value={row.quantity}
                        onChange={(e) => updateRow(globalIndex, 'quantity', parseInt(e.target.value, 10) || 1)}
                        className="w-14 px-2 py-1 border border-gray-200 rounded text-xs text-right"
                      />
                    </td>
                    <td className="py-1.5 pr-2">
                      <select
                        value={row.toolType}
                        onChange={(e) => updateRow(globalIndex, 'toolType', e.target.value)}
                        className="w-full px-2 py-1 border border-gray-200 rounded text-xs"
                      >
                        {['Standard', 'Special', 'Hired', 'Consumable'].map((opt) => (
                    ...
                    <td className="py-1.5 pr-2 text-center">
                      <input
                        type="checkbox"
                        checked={row.certRequired}
                        onChange={(e) => updateRow(globalIndex, 'certRequired', e.target.checked)}
                        className="rounded"
                      />
                    </td>
                  </tr>
        ...
        <div className="flex items-center justify-end gap-3 p-4 border-t border-gray-200 flex-shrink-0">
          ...
          >
            {saving ? 'Saving…' : `✓ Add ${selectedCount} Tools to List`}
          </button>
        </div>
      </div>
    </div>
