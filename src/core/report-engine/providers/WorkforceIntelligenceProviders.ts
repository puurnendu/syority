...
class TodayAttendanceProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    ...
    const records = await db.daily_attendance.findMany({
    ...
    const total = records.reduce((s: number, r: any) => s + (r.headcount ?? 0), 0);
...
class ContractorAttendanceProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    const today = new Date().toISOString().slice(0, 10);
    const records = await db.daily_attendance.groupBy({
      by: ['contractor_id'],
      where: { organization_id: ctx.organizationId, attendance_date: new Date(today), ...eventScope(params) },
      _sum: { headcount: true },
    }).catch(() => []);
    return { rows: records.map((r: any) => ({ contractorId: r.contractor_id, headcount: r._sum?.headcount ?? 0 })) };
  }
}
...
class DisciplineAttendanceProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    ...
    return { rows: records.map((r: any) => ({ discipline: r.discipline, headcount: r._sum?.headcount ?? 0 })) };
  }
}
...
class ShiftAttendanceProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    ...
    return { rows: records.map((r: any) => ({ shift: r.shift, headcount: r._sum?.headcount ?? 0 })) };
  }
}
...
class CrewAvailabilityProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    const today = new Date().toISOString().slice(0, 10);
    const actual = await db.daily_attendance.aggregate({
      where: { organization_id: ctx.organizationId, attendance_date: new Date(today), ...eventScope(params) },
      _sum: { headcount: true },
    }).catch(() => ({ _sum: { headcount: 0 } }));
    const planned = await db.activity.aggregate({
...
class OvertimeProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    const records = await db.daily_attendance.aggregate({
      where: { organization_id: ctx.organizationId, ...eventScope(params) },
      _sum: { overtime_hours: true, regular_hours: true },
    }).catch(() => ({ _sum: { overtime_hours: 0, regular_hours: 0 } }));
    return {
      kpis: [
...
class AbsenteeismProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    const today = new Date().toISOString().slice(0, 10);
    const att = await db.daily_attendance.aggregate({
      where: { organization_id: ctx.organizationId, attendance_date: new Date(today), ...eventScope(params) },
      _sum: { headcount: true, absent_count: true },
    }).catch(() => ({ _sum: { headcount: 0, absent_count: 0 } }));
...
class ProductivityProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    ...
    }).catch(() => ({ _sum: { regular_hours: 0, productive_hours: 0 } }));
...
class SafetyManhourProvider extends BaseProvider {
  ...
  async fetch(ctx: ProviderContext, params: Record<string, any>): Promise<DataFetcherResult> {
    const total = await db.daily_attendance.aggregate({
      where: { organization_id: ctx.organizationId, ...eventScope(params) },
      _sum: { regular_hours: true, overtime_hours: true },
    }).catch(() => ({ _sum: { regular_hours: 0, overtime_hours: 0 } }));
